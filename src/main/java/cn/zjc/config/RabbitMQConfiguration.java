...
public class RabbitMQConfiguration {
    ...
    public RabbitTemplate rabbitTemplate(CachingConnectionFactory connectionFactory,
        ...
        rabbitTemplate.setConfirmCallback(new RabbitTemplate.ConfirmCallback() {
            ...
            public void confirm(CorrelationData correlationData, boolean ack, String cause) {
                if (ack) {   //消息发布成功,为要发布的消息添加缓存,成功则删除缓存
                    ...
                    jedisStringService.del(correlationData.getId());  //删除缓存中的数据
                } else {   //消息发布失败,通过缓存获取消息并进行失败处理
                    log.debug("消息发布失败,CorrelationData id==>" + correlationData.getId() + ";cause:" + cause);
                }
            }
        });
        ...
        //错误路由的话一般不应该出现,属于比较严重的逻辑错误,采用重新推送到x-exception-message队列
